...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (socket_desc == ERROR) {
        throw domain_error(LineInfo("Socket creation failure ", __FILE__, __LINE__));
    }
    ...
    if((listen(socket_desc,LISTEN_QUEUE_SIZE)))
        ...
        if (guessnostr.find("Random Number") != std::string::npos) {
           ...
           if      (read_size > 0) {
               ...
               //write to client_sock playresultstr 
                  int result = write(client_sock, playresultstr.c_str(), playresultstr.size());
           if (result == ERROR) {
              throw domain_error(LineInfo("Socket write failure ", __FILE__, __LINE__));
           }
           memset(clientGuessNoMessage, 0, MESSAGE_MAX_SIZE);  //clearing the buffer for the next guess
           }
